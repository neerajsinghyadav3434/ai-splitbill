...
export const getBirthdayUsers = query({
  handler: async (ctx) => {
    const users = await ctx.db.query("users").collect();
    ...
    // Format matches MM-DD from YYYY-MM-DD string
    const target = `${month.toString().padStart(2, "0")}-${day.toString().padStart(2, "0")}`;
    ...
    return users.filter((u) => {
      ...
      return parts[1] === target.split("-")[0] && parts[2] === target.split("-")[1];
    });
  },
});

// Get contacts for a specific user (system query)
export const getContactsForUser = query({
  args: { userId: v.id("users") },
  handler: async (ctx, args) => {
    ...
    // 2. Collect all unique user IDs (excluding target user)
    ...
    const contacts = await Promise.all(
      Array.from(contactIds).map((id) => ctx.db.get(id))
    );
